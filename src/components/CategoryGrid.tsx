  ...
  return (
    <section id="categories" className="py-12 md:py-16 bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 8 Category Grid (4 cols x 2 rows) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {CATEGORIES.map((category) => (
            <div
              key={category.id}
              onClick={() => onSelectCategory(category)}
              className="group relative h-48 sm:h-52 rounded-2xl overflow-hidden shadow-xs hover:shadow-lg transition-all duration-300 cursor-pointer bg-slate-900 border-t-4 border-rose-600"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 z-10 flex flex-col justify-end">
                ...
                <p className="text-slate-300 text-xs mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1">
